...
inline std::ostream& operator<<(std::ostream& out, const mfxFrameInfo& info)
{
    out << "FrameInfo: " << std::endl
        ...
        << "| ROI: "
        << "(" << info.CropX << ";" << info.CropY << ") " << info.CropW << "x" << info.CropH << std::endl
...
public:
    ...
    {
        static const mfxPluginUID hevc_enc_uid = {0x6f, 0xad, 0xc7, 0x91, 0xa0, 0xc2, 0xeb, 0x47,
                                                  0x9a, 0xb6, 0xdc, 0xd5, 0xea, 0x9d, 0xa3, 0x47};
    ...
    {
        static const mfxPluginUID hevc_dec_uid = {0x33, 0xa6, 0x1c, 0x0b, 0x4c, 0x27, 0x45, 0x4c,
                                                  0xa8, 0xd8, 0x5d, 0xde, 0x75, 0x7c, 0x6f, 0x8e};
        if (codecId == MFX_CODEC_HEVC)
...
public:
    ...
    static SurfacePool* create(T* instance, mfxVideoParam& params)
    {
        ...
        if (res < MFX_ERR_NONE)
            return 0;
        return new SurfacePool(request.Info.Width, request.Info.Height, request.NumFrameSuggested,
