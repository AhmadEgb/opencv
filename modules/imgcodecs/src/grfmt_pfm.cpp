...
void swap_endianess(uint32_t& ui)
{
    ...
    ui = ((ui & A) << 24) | ((ui & B) << 8) | ((ui & C) >> 8) | ((ui & D) >> 24);
}

template<typename T>
T atoT(const std::string& s);
template<>
int atoT<int>(const std::string& s)
{
    return std::atoi(s.c_str());
}
template<>
double atoT<double>(const std::string& s)
{
...
template<typename T>
void write_anything(cv::WLByteStream& strm, const T& t)
{
...
PFMDecoder::PFMDecoder() { m_strm.close(); }
...
size_t PFMDecoder::signatureLength() const { return 3; }

bool PFMDecoder::checkSignature(const String& signature) const
{
    return signature.size() >= 3 && signature[0] == 'P' && (signature[1] == 'f' || signature[1] == 'F')
           && isspace(signature[2]);
}
...
PFMEncoder::PFMEncoder() { m_description = "Portable image format - float (*.pfm)"; }
