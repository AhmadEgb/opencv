...
struct RGB2YCrCb_f<float>
{
    ...
    void process(__m128 v_r, __m128 v_g, __m128 v_b, __m128& v_y, __m128& v_cr, __m128& v_cb) const
    {
    ...
    void operator()(const float* src, float* dst, int n) const
    {
        ...
        if (haveSIMD)
        {
            for (; i <= n - 24; i += 24, src += 8 * scn)
            {
                ...
                process(v_r0, v_g0, v_b0, v_y0, v_cr0, v_cb0);

                __m128 v_y1, v_cr1, v_cb1;
                process(v_r1, v_g1, v_b1, v_y1, v_cr1, v_cb1);
...
struct RGB2YCrCb_i
{
    ...
    RGB2YCrCb_i(int _srccn, int _blueIdx, bool _isCrCb) : srccn(_srccn), blueIdx(_blueIdx), isCrCb(_isCrCb)
    {
...
struct RGB2YCrCb_i<uchar>
{
    ...
    void operator()(const uchar* src, uchar* dst, int n) const
    {
        ...
        for (; i <= n - 24; i += 24, src += scn * 8)
        {
            ...
            int32x4_t v_Y0 = vmlal_s16(vmlal_s16(vmull_s16(v_src0.val[0], v_c0), v_src0.val[1], v_c1),
                                       v_src0.val[2], v_c2);
            ...
            int32x4_t v_Y1 = vmlal_s16(vmlal_s16(vmull_s16(v_src0.val[0], v_c0), v_src0.val[1], v_c1),
                                       v_src0.val[2], v_c2);
...
struct RGB2YCrCb_i<ushort>
{
    ...
    void operator()(const ushort* src, ushort* dst, int n) const
    {
        ...
        for (; i <= n - 24; i += 24, src += scn * 8)
        {
            ...
            int32x4_t v_Y0 = vmlaq_s32(vmlaq_s32(vmulq_s32(v_src0.val[0], v_c0), v_src0.val[1], v_c1),
                                       v_src0.val[2], v_c2);
            v_Y0 = vshrq_n_s32(vaddq_s32(v_Y0, v_delta2), yuv_shift);
            ...
            int32x4_t v_Y1 = vmlaq_s32(vmlaq_s32(vmulq_s32(v_src0.val[0], v_c0), v_src0.val[1], v_c1),
                                       v_src0.val[2], v_c2);
            v_Y1 = vshrq_n_s32(vaddq_s32(v_Y1, v_delta2), yuv_shift);
        ...
        for (; i <= n - 12; i += 12, src += scn * 4)
        {
            ...
            int32x4_t v_Y = vmlaq_s32(vmlaq_s32(vmulq_s32(v_src0.val[0], v_c0), v_src0.val[1], v_c1),
                                      v_src0.val[2], v_c2);
...
struct RGB2YCrCb_i<uchar>
{
    ...
    RGB2YCrCb_i(int _srccn, int _blueIdx, bool _isCrCb) : srccn(_srccn), blueIdx(_blueIdx), isCrCb(_isCrCb)
    {
        ...
        v_coeff = _mm_set_epi16(delta2, (short)coeffs[4], delta2, (short)coeffs[3], delta2, (short)coeffs[4],
                                delta2, (short)coeffs[3]);
        if (isCrCb)
            v_shuffle2 = _mm_set_epi8(0x0, 0x0, 0x0, 0x0, 0xf, 0xe, 0xc, 0xb, 0xa, 0x8, 0x7, 0x6, 0x4, 0x3, 0x2,
                                      0x0);
        else //if YUV
            v_shuffle2 = _mm_set_epi8(0x0, 0x0, 0x0, 0x0, 0xe, 0xf, 0xc, 0xa, 0xb, 0x8, 0x6, 0x7, 0x4, 0x2, 0x3,
                                      0x0);
        haveSIMD = checkHardwareSupport(CV_CPU_SSE4_1);
    }

    // 16u x 8
    void process(__m128i* v_rgb, __m128i& v_crgb, __m128i* v_rb, uchar* dst) const
    {
    ...
    void operator()(const uchar* src, uchar* dst, int n) const
    {
        ...
        if (haveSIMD)
        {
            ...
            if (scn == 4)
            {
                if (bidx == 0)
                {
                    v_shuffle = _mm_set_epi8(0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xc, 0xe, 0x8, 0xa, 0x4, 0x6,
                                             0x0, 0x2);
                }
                else
                {
                    v_shuffle = _mm_set_epi8(0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xe, 0xc, 0xa, 0x8, 0x6, 0x4,
                                             0x2, 0x0);
                }
            ...
            else
            {
                if (bidx == 0)
                {
                    v_shuffle = _mm_set_epi8(0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x9, 0xb, 0x6, 0x8, 0x3, 0x5,
                                             0x0, 0x2);
                }
                else
                {
                    v_shuffle = _mm_set_epi8(0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xb, 0x9, 0x8, 0x6, 0x5, 0x3,
                                             0x2, 0x0);
                }
...
struct RGB2YCrCb_i<ushort>
{
    ...
    // 16u x 8
    void process(__m128i v_r, __m128i v_g, __m128i v_b, __m128i& v_y, __m128i& v_cr, __m128i& v_cb) const
    {
        ...
        __m128i v_y0 = _mm_add_epi32(_mm_mullo_epi32(v_r_p, v_c0),
                                     _mm_add_epi32(_mm_mullo_epi32(v_g_p, v_c1), _mm_mullo_epi32(v_b_p, v_c2)));
...
struct YCrCb2RGB_f
{
    ...
    YCrCb2RGB_f(int _dstcn, int _blueIdx, bool _isCrCb) : dstcn(_dstcn), blueIdx(_blueIdx), isCrCb(_isCrCb)
    {
        ...
        if (dcn == 3)
            for (; i <= n - 12; i += 12, dst += 12)
            {
                ...
                v_dst.val[1] = vaddq_f32(
                    vmlaq_f32(vmulq_f32(vsubq_f32(v_Cb, v_delta), v_c2), vsubq_f32(v_Cr, v_delta), v_c1), v_Y);
...
struct YCrCb2RGB_f<float>
{
    ...
    void process(__m128 v_y, __m128 v_cr, __m128 v_cb, __m128& v_r, __m128& v_g, __m128& v_b) const
    {
        ...
        if (haveSIMD)
        {
            for (; i <= n - 24; i += 24, dst += 8 * dcn)
            {
                ...
                if (dcn == 3)
                    ...
                    _mm_interleave_ps(v_r0, v_r1, v_g0, v_g1, v_b0, v_b1, v_a0, v_a1);
...
struct YCrCb2RGB_i
{
    ...
    YCrCb2RGB_i(int _dstcn, int _blueIdx, bool _isCrCb) : dstcn(_dstcn), blueIdx(_blueIdx), isCrCb(_isCrCb)
    {
...
struct YCrCb2RGB_i<uchar>
{
    ...
    void operator()(const uchar* src, uchar* dst, int n) const
    {
        ...
        for (; i <= n - 24; i += 24, dst += dcn * 8)
        {
            ...
            int16x4_t v_Y = vget_low_s16(v_src16.val[0]), v_Cr = vget_low_s16(v_src16.val[1 + yuvOrder]),
...
#    if CV_SSE4_1
    // 16s x 8
    void process(__m128i* v_src, __m128i* v_shuffle, __m128i* v_coeffs) const
    {
        __m128i v_ycrcb[3];
...
#    endif // CV_SSE4_1
    ...
    // 16s x 8
    void process(__m128i v_y, __m128i v_cr, __m128i v_cb, __m128i& v_r, __m128i& v_g, __m128i& v_b) const
    {
        ...
        __m128i v_g0 = _mm_srai_epi32(_mm_add_epi32(_mm_add_epi32(_mm_unpacklo_epi16(v_mullo_2, v_mulhi_2),
                                                                  _mm_unpacklo_epi16(v_mullo_1, v_mulhi_1)),
                                                    v_delta2),
                                      yuv_shift);
        ...
        __m128i v_g1 = _mm_srai_epi32(_mm_add_epi32(_mm_add_epi32(_mm_unpackhi_epi16(v_mullo_2, v_mulhi_2),
                                                                  _mm_unpackhi_epi16(v_mullo_1, v_mulhi_1)),
                                                    v_delta2),
                                      yuv_shift);
...
#    if CV_SSE4_1
        ...
        {
            __m128i v_shuffle[2];
            v_shuffle[0] = _mm_set_epi8(0x8, 0x7, 0x7, 0x6, 0x6, 0x5, 0x5, 0x4, 0x4, 0x3, 0x3, 0x2, 0x2, 0x1, 0x1,
                                        0x0);
            v_shuffle[1] = _mm_set_epi8(0xf, 0xc, 0xc, 0xc, 0x9, 0x9, 0x9, 0x6, 0x6, 0x6, 0x3, 0x3, 0x3, 0x0, 0x0,
                                        0x0);
            __m128i v_coeffs[3];
            ...
            if (dcn == 3)
            {
                if (bidx == 0)
                {
                    __m128i v_shuffle_dst = _mm_set_epi8(0xf, 0xc, 0xd, 0xe, 0x9, 0xa, 0xb, 0x6, 0x7, 0x8, 0x3,
                                                         0x4, 0x5, 0x0, 0x1, 0x2);
                    for (; i <= n - 24; i += 24, dst += dcn * 8)
                    {
                        __m128i v_src[2];
            ...
            else
            {
                if (bidx == 0)
                {
                    __m128i v_shuffle_dst = _mm_set_epi8(0x0, 0xa, 0xb, 0xc, 0x0, 0x7, 0x8, 0x9, 0x0, 0x4, 0x5,
                                                         0x6, 0x0, 0x1, 0x2, 0x3);
                    ...
                    for (; i <= n - 24; i += 24, dst += dcn * 8)
                    {
                        ...
                        _mm_storeu_si128((__m128i*)(dst),
                                         _mm_shuffle_epi8(_mm_alignr_epi8(v_src[0], v_alpha, 15), v_shuffle_dst));
                        _mm_storeu_si128((__m128i*)(dst + 16),
                                         _mm_shuffle_epi8(_mm_alignr_epi8(_mm_alignr_epi8(v_src[1], v_src[0], 12),
                                                                          v_alpha, 15),
                                                          v_shuffle_dst));
                    }
                }
                ...
                {
                    __m128i v_shuffle_dst = _mm_set_epi8(0x0, 0xc, 0xb, 0xa, 0x0, 0x9, 0x8, 0x7, 0x0, 0x6, 0x5,
                                                         0x4, 0x0, 0x3, 0x2, 0x1);
...
#    endif // CV_SSE4_1
            if (haveSIMD && useSSE)
        {
            for (; i <= n - 96; i += 96, dst += dcn * 32)
            {
                ...
                process(_mm_unpacklo_epi8(v_y0, v_zero), _mm_unpacklo_epi8(v_cr0, v_zero),
                        _mm_unpacklo_epi8(v_cb0, v_zero), v_r_0, v_g_0, v_b_0);

                __m128i v_r_1 = v_zero, v_g_1 = v_zero, v_b_1 = v_zero;
                process(_mm_unpackhi_epi8(v_y0, v_zero), _mm_unpackhi_epi8(v_cr0, v_zero),
                        _mm_unpackhi_epi8(v_cb0, v_zero), v_r_1, v_g_1, v_b_1);
                ...
                process(_mm_unpacklo_epi8(v_y1, v_zero), _mm_unpacklo_epi8(v_cr1, v_zero),
                        _mm_unpacklo_epi8(v_cb1, v_zero), v_r_0, v_g_0, v_b_0);

                process(_mm_unpackhi_epi8(v_y1, v_zero), _mm_unpackhi_epi8(v_cr1, v_zero),
                        _mm_unpackhi_epi8(v_cb1, v_zero), v_r_1, v_g_1, v_b_1);
...
struct YUV420sp2RGB888Invoker : ParallelLoopBody
{
    ...
    YUV420sp2RGB888Invoker(uchar* _dst_data, size_t _dst_step, int _dst_width, size_t _stride, const uchar* _y1,
                           const uchar* _uv)
...
struct YUV420sp2RGBA8888Invoker : ParallelLoopBody
{
    ...
    YUV420sp2RGBA8888Invoker(uchar* _dst_data, size_t _dst_step, int _dst_width, size_t _stride, const uchar* _y1,
                             const uchar* _uv)
        : dst_data(_dst_data), dst_step(_dst_step), width(_dst_width), my1(_y1), muv(_uv), stride(_stride)
    {
    }
...
struct YUV420p2RGB888Invoker : ParallelLoopBody
{
    ...
    YUV420p2RGB888Invoker(uchar* _dst_data, size_t _dst_step, int _dst_width, size_t _stride, const uchar* _y1,
                          const uchar* _u, const uchar* _v, int _ustepIdx, int _vstepIdx)
        : dst_data(_dst_data), dst_step(_dst_step), width(_dst_width), my1(_y1), mu(_u), mv(_v), stride(_stride),
          ustepIdx(_ustepIdx), vstepIdx(_vstepIdx)
    {
    }
...
struct YUV420p2RGBA8888Invoker : ParallelLoopBody
{
    ...
    YUV420p2RGBA8888Invoker(uchar* _dst_data, size_t _dst_step, int _dst_width, size_t _stride, const uchar* _y1,
                            const uchar* _u, const uchar* _v, int _ustepIdx, int _vstepIdx)
        : dst_data(_dst_data), dst_step(_dst_step), width(_dst_width), my1(_y1), mu(_u), mv(_v), stride(_stride),
          ustepIdx(_ustepIdx), vstepIdx(_vstepIdx)
    {
...
template<int bIdx, int uIdx>
inline void cvtYUV420sp2RGB(uchar* dst_data, size_t dst_step, int dst_width, int dst_height, size_t _stride,
...
template<int bIdx, int uIdx>
inline void cvtYUV420sp2RGBA(uchar* dst_data, size_t dst_step, int dst_width, int dst_height, size_t _stride,
                             const uchar* _y1, const uchar* _uv)
{
...
template<int bIdx>
inline void cvtYUV420p2RGB(uchar* dst_data, size_t dst_step, int dst_width, int dst_height, size_t _stride,
                           const uchar* _y1, const uchar* _u, const uchar* _v, int ustepIdx, int vstepIdx)
{
...
template<int bIdx>
inline void cvtYUV420p2RGBA(uchar* dst_data, size_t dst_step, int dst_width, int dst_height, size_t _stride,
                            const uchar* _y1, const uchar* _u, const uchar* _v, int ustepIdx, int vstepIdx)
{
...
{
    RGB888toYUV420pInvoker(const uchar* _src_data, size_t _src_step, uchar* _y_data, uchar* _uv_data,
                           size_t _dst_step, int _src_width, int _src_height, int _scn, bool swapBlue_,
                           bool swapUV_, bool interleaved_)
        : src_data(_src_data), src_step(_src_step), y_data(_y_data), uv_data(_uv_data), dst_step(_dst_step),
          src_width(_src_width), src_height(_src_height), scn(_scn), swapBlue(swapBlue_), swapUV(swapUV_),
    ...
    void operator()(const Range& rowRange) const CV_OVERRIDE
    {
        ...
        for (int i = rowRange.start; i < rowRange.end; i++)
        {
            ...
            for (int j = 0, k = 0; j < w * cn; j += 2 * cn, k++)
            {
                ...
                int y00 = ITUR_BT_601_CRY * r00 + ITUR_BT_601_CGY * g00 + ITUR_BT_601_CBY * b00 + halfShift
                          + shifted16;
                int y01 = ITUR_BT_601_CRY * r01 + ITUR_BT_601_CGY * g01 + ITUR_BT_601_CBY * b01 + halfShift
                          + shifted16;
                int y10 = ITUR_BT_601_CRY * r10 + ITUR_BT_601_CGY * g10 + ITUR_BT_601_CBY * b10 + halfShift
                          + shifted16;
                int y11 = ITUR_BT_601_CRY * r11 + ITUR_BT_601_CGY * g11 + ITUR_BT_601_CBY * b11 + halfShift
                          + shifted16;
                ...
                int u00 = ITUR_BT_601_CRU * r00 + ITUR_BT_601_CGU * g00 + ITUR_BT_601_CBU * b00 + halfShift
                          + shifted128;
                int v00 = ITUR_BT_601_CBU * r00 + ITUR_BT_601_CGV * g00 + ITUR_BT_601_CBV * b00 + halfShift
                          + shifted128;
...
struct YUV422toRGB888Invoker : ParallelLoopBody
{
    ...
    YUV422toRGB888Invoker(uchar* _dst_data, size_t _dst_step, const uchar* _src_data, size_t _src_step, int _width)
...
struct YUV422toRGBA8888Invoker : ParallelLoopBody
{
    ...
    YUV422toRGBA8888Invoker(uchar* _dst_data, size_t _dst_step, const uchar* _src_data, size_t _src_step, int _width)
        : dst_data(_dst_data), dst_step(_dst_step), src_data(_src_data), src_step(_src_step), width(_width)
    {
    }
...
template<int bIdx, int uIdx, int yIdx>
inline void cvtYUV422toRGB(uchar* dst_data, size_t dst_step, const uchar* src_data, size_t src_step, int width,
                           int height)
{
...
template<int bIdx, int uIdx, int yIdx>
inline void cvtYUV422toRGBA(uchar* dst_data, size_t dst_step, const uchar* src_data, size_t src_step, int width,
                            int height)
{
...
// 8u, 16u, 32f
void cvtBGRtoYUV(const uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width, int height,
                 int depth, int scn, bool swapBlue, bool isCbCr)
{
    CV_INSTRUMENT_REGION();

    CALL_HAL(cvtBGRtoYUV, cv_hal_cvtBGRtoYUV, src_data, src_step, dst_data, dst_step, width, height, depth, scn,
             swapBlue, isCbCr);
...
#    endif
    ...
    if (depth == CV_8U)
        CvtColorLoop(src_data, src_step, dst_data, dst_step, width, height,
                     RGB2YCrCb_i<uchar>(scn, blueIdx, isCbCr));
    else if (depth == CV_16U)
        CvtColorLoop(src_data, src_step, dst_data, dst_step, width, height,
                     RGB2YCrCb_i<ushort>(scn, blueIdx, isCbCr));
    else
        CvtColorLoop(src_data, src_step, dst_data, dst_step, width, height,
                     RGB2YCrCb_f<float>(scn, blueIdx, isCbCr));
}

void cvtYUVtoBGR(const uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width, int height,
                 int depth, int dcn, bool swapBlue, bool isCbCr)
{
    CV_INSTRUMENT_REGION();

    CALL_HAL(cvtYUVtoBGR, cv_hal_cvtYUVtoBGR, src_data, src_step, dst_data, dst_step, width, height, depth, dcn,
             swapBlue, isCbCr);
...
#    endif
    ...
    if (depth == CV_8U)
        CvtColorLoop(src_data, src_step, dst_data, dst_step, width, height,
                     YCrCb2RGB_i<uchar>(dcn, blueIdx, isCbCr));
    else if (depth == CV_16U)
        CvtColorLoop(src_data, src_step, dst_data, dst_step, width, height,
                     YCrCb2RGB_i<ushort>(dcn, blueIdx, isCbCr));
    else
        CvtColorLoop(src_data, src_step, dst_data, dst_step, width, height,
                     YCrCb2RGB_f<float>(dcn, blueIdx, isCbCr));
}

void cvtTwoPlaneYUVtoBGR(const uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int dst_width,
                         int dst_height, int dcn, bool swapBlue, int uIdx)
{
    CV_INSTRUMENT_REGION();

    CALL_HAL(cvtTwoPlaneYUVtoBGR, cv_hal_cvtTwoPlaneYUVtoBGR, src_data, src_step, dst_data, dst_step, dst_width,
             dst_height, dcn, swapBlue, uIdx);
...
void cvtTwoPlaneYUVtoBGR(const uchar* y_data, const uchar* uv_data, size_t src_step, uchar* dst_data,
                         size_t dst_step, int dst_width, int dst_height, int dcn, bool swapBlue, int uIdx)
{
    CV_INSTRUMENT_REGION();
    ...
    {
    case 300:
        cvtYUV420sp2RGB<0, 0>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 301:
        cvtYUV420sp2RGB<0, 1>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 320:
        cvtYUV420sp2RGB<2, 0>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 321:
        cvtYUV420sp2RGB<2, 1>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 400:
        cvtYUV420sp2RGBA<0, 0>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 401:
        cvtYUV420sp2RGBA<0, 1>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 420:
        cvtYUV420sp2RGBA<2, 0>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    case 421:
        cvtYUV420sp2RGBA<2, 1>(dst_data, dst_step, dst_width, dst_height, src_step, y_data, uv_data);
        break;
    default:
...
void cvtThreePlaneYUVtoBGR(const uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int dst_width,
                           int dst_height, int dcn, bool swapBlue, int uIdx)
{
    CV_INSTRUMENT_REGION();

    CALL_HAL(cvtThreePlaneYUVtoBGR, cv_hal_cvtThreePlaneYUVtoBGR, src_data, src_step, dst_data, dst_step,
    ...
    {
    case 30:
        cvtYUV420p2RGB<0>(dst_data, dst_step, dst_width, dst_height, src_step, src_data, u, v, ustepIdx, vstepIdx);
        break;
    case 32:
        cvtYUV420p2RGB<2>(dst_data, dst_step, dst_width, dst_height, src_step, src_data, u, v, ustepIdx, vstepIdx);
        break;
    case 40:
        cvtYUV420p2RGBA<0>(dst_data, dst_step, dst_width, dst_height, src_step, src_data, u, v, ustepIdx, vstepIdx);
        break;
    case 42:
        cvtYUV420p2RGBA<2>(dst_data, dst_step, dst_width, dst_height, src_step, src_data, u, v, ustepIdx, vstepIdx);
        break;
    default:
        CV_Error(CV_StsBadFlag, "Unknown/unsupported color conversion code");
        break;
...
void cvtBGRtoThreePlaneYUV(const uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width,
                           int height, int scn, bool swapBlue, int uIdx)
{
    CV_INSTRUMENT_REGION();

    CALL_HAL(cvtBGRtoThreePlaneYUV, cv_hal_cvtBGRtoThreePlaneYUV, src_data, src_step, dst_data, dst_step, width,
             height, scn, swapBlue, uIdx);
    uchar* uv_data = dst_data + dst_step * height;
    RGB888toYUV420pInvoker(src_data, src_step, dst_data, uv_data, dst_step, width, height, scn, swapBlue,
                           uIdx == 2, false)
        .convert();
}

void cvtBGRtoTwoPlaneYUV(const uchar* src_data, size_t src_step, uchar* y_data, uchar* uv_data, size_t dst_step,
                         int width, int height, int scn, bool swapBlue, int uIdx)
{
    CV_INSTRUMENT_REGION();

    // TODO: add hal replacement method
    RGB888toYUV420pInvoker(src_data, src_step, y_data, uv_data, dst_step, width, height, scn, swapBlue, uIdx == 2,
                           true)
        .convert();
}

void cvtOnePlaneYUVtoBGR(const uchar* src_data, size_t src_step, uchar* dst_data, size_t dst_step, int width,
                         int height, int dcn, bool swapBlue, int uIdx, int ycn)
{
    CV_INSTRUMENT_REGION();

    CALL_HAL(cvtOnePlaneYUVtoBGR, cv_hal_cvtOnePlaneYUVtoBGR, src_data, src_step, dst_data, dst_step, width,
             height, dcn, swapBlue, uIdx, ycn);
    int blueIdx = swapBlue ? 2 : 0;
    switch (dcn * 1000 + blueIdx * 100 + uIdx * 10 + ycn)
    {
    case 3000:
        cvtYUV422toRGB<0, 0, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 3001:
        cvtYUV422toRGB<0, 0, 1>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 3010:
        cvtYUV422toRGB<0, 1, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 3200:
        cvtYUV422toRGB<2, 0, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 3201:
        cvtYUV422toRGB<2, 0, 1>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 3210:
        cvtYUV422toRGB<2, 1, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 4000:
        cvtYUV422toRGBA<0, 0, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 4001:
        cvtYUV422toRGBA<0, 0, 1>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 4010:
        cvtYUV422toRGBA<0, 1, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 4200:
        cvtYUV422toRGBA<2, 0, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 4201:
        cvtYUV422toRGBA<2, 0, 1>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    case 4210:
        cvtYUV422toRGBA<2, 1, 0>(dst_data, dst_step, src_data, src_step, width, height);
        break;
    default:
        CV_Error(CV_StsBadFlag, "Unknown/unsupported color conversion code");
        break;
...
bool oclCvtColorYUV2BGR(InputArray _src, OutputArray _dst, int dcn, int bidx)
{
    ...
    if (!h.createKernel("YUV2RGB", ocl::imgproc::color_yuv_oclsrc, format("-D dcn=%d -D bidx=%d", dcn, bidx)))
    {
        return false;
    }
...
bool oclCvtColorBGR2YUV(InputArray _src, OutputArray _dst, int bidx)
{
    ...
    if (!h.createKernel("RGB2YUV", ocl::imgproc::color_yuv_oclsrc, format("-D dcn=3 -D bidx=%d", bidx)))
    {
        return false;
    }
...
bool oclCvtcolorYCrCb2BGR(InputArray _src, OutputArray _dst, int dcn, int bidx)
{
    ...
    if (!h.createKernel("YCrCb2RGB", ocl::imgproc::color_yuv_oclsrc, format("-D dcn=%d -D bidx=%d", dcn, bidx)))
    {
        return false;
    }
...
bool oclCvtColorBGR2YCrCb(InputArray _src, OutputArray _dst, int bidx)
{
    ...
    if (!h.createKernel("RGB2YCrCb", ocl::imgproc::color_yuv_oclsrc, format("-D dcn=3 -D bidx=%d", bidx)))
    {
        return false;
    }
...
void cvtColorBGR2YUV(InputArray _src, OutputArray _dst, bool swapb, bool crcb)
{
    ...
    hal::cvtBGRtoYUV(h.src.data, h.src.step, h.dst.data, h.dst.step, h.src.cols, h.src.rows, h.depth, h.scn, swapb,
                     crcb);
}
...
void cvtColorYUV2BGR(InputArray _src, OutputArray _dst, int dcn, bool swapb, bool crcb)
{
    ...
    hal::cvtYUVtoBGR(h.src.data, h.src.step, h.dst.data, h.dst.step, h.src.cols, h.src.rows, h.depth, dcn, swapb,
                     crcb);
}
...
void cvtColorOnePlaneYUV2BGR(InputArray _src, OutputArray _dst, int dcn, bool swapb, int uidx, int ycn)
{
    ...
    hal::cvtOnePlaneYUVtoBGR(h.src.data, h.src.step, h.dst.data, h.dst.step, h.src.cols, h.src.rows, dcn, swapb,
                             uidx, ycn);
}
...
void cvtColorBGR2ThreePlaneYUV(InputArray _src, OutputArray _dst, bool swapb, int uidx)
{
    ...
    hal::cvtBGRtoThreePlaneYUV(h.src.data, h.src.step, h.dst.data, h.dst.step, h.src.cols, h.src.rows, h.scn,
                               swapb, uidx);
}
...
#    if IPP_VERSION_X100 >= 201700
    if (CV_INSTRUMENT_FUN_IPP(ippiCopy_8u_C1R_L, h.src.data, (IppSizeL)h.src.step, h.dst.data,
                              (IppSizeL)h.dst.step, ippiSizeL(h.dstSz.width, h.dstSz.height))
        >= 0)
        return;
#    endif
#endif
...
void cvtColorThreePlaneYUV2BGR(InputArray _src, OutputArray _dst, int dcn, bool swapb, int uidx)
{
    ...
    hal::cvtThreePlaneYUVtoBGR(h.src.data, h.src.step, h.dst.data, h.dst.step, h.dst.cols, h.dst.rows, dcn, swapb,
                               uidx);
}
...
void cvtColorTwoPlaneYUV2BGR(InputArray _src, OutputArray _dst, int dcn, bool swapb, int uidx)
{
    ...
    hal::cvtTwoPlaneYUVtoBGR(h.src.data, h.src.step, h.dst.data, h.dst.step, h.dst.cols, h.dst.rows, dcn, swapb,
                             uidx);
}
...
void cvtColorTwoPlaneYUV2BGRpair(InputArray _ysrc, InputArray _uvsrc, OutputArray _dst, int dcn, bool swapb, int uidx)
{
    ...
    hal::cvtTwoPlaneYUVtoBGR(ysrc.data, uvsrc.data, ysrc.step, dst.data, dst.step, dst.cols, dst.rows, dcn, swapb,
                             uidx);
}
