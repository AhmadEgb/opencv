...
class ONNXImporter
{
    ...
    std::map<std::string, Mat> getGraphTensors(const opencv_onnx::GraphProto& graph_proto);
...
LayerParams ONNXImporter::getLayerParams(const opencv_onnx::NodeProto& node_proto)
{
    ...
    for (int i = 0; i < node_proto.attribute_size(); i++)
    {
        ...
        {
            lp.set(attribute_name,
                   DictValue::arrayReal(attribute_proto.floats().data(), attribute_proto.floats_size()));
        }
...
Mat ONNXImporter::getBlob(const opencv_onnx::NodeProto& node_proto, const std::map<std::string, Mat>& constBlobs,
                          int index)
{
    ...
    {
        CV_Error(Error::StsObjectNotFound, "Blob " + node_proto.input(index) + " not found in const blobs");
    }
    ...
    for (int i = 0; i < layersSize; i++)
    {
        ...
        else if (layer_type == "ImageScaler")
        {
            ...
            if (layerParams.has("bias"))
            {
                ...
                layerParams.blobs.push_back(Mat(Size(1, layerParams.get("bias").size()), CV_32FC1, scale));
