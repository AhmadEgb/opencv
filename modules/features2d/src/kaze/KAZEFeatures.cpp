 ...
 */
KAZEFeatures::KAZEFeatures(KAZEOptions& options) : options_(options)
{
    ncycles_ = 0;
    reordering_ = true;
...
{
    options_.kcontrast = compute_k_percentile(img, kpercentile, options_.sderivatives, options_.kcontrast_bins, 0,
                                              0);
}
...
{
    parallel_for_(Range(0, (int)evolution_.size()), MultiscaleDerivativesKAZEInvoker(evolution_));
}
...
public:
    explicit FindExtremumKAZEInvoker(std::vector<TEvolution>& ev, std::vector<std::vector<KeyPoint>>& kpts_par,
        ...
        : evolution_(&ev), kpts_par_(&kpts_par), options_(options)
    {
    }
...
void KAZEFeatures::Determinant_Hessian(std::vector<KeyPoint>& kpts)
{
    ...
    parallel_for_(Range(1, (int)evolution_.size() - 1), FindExtremumKAZEInvoker(evolution_, kpts_par_, options_));
    ...
    for (int i = 0; i < (int)kpts_par_.size(); i++)
    {
        for (int j = 0; j < (int)kpts_par_[i].size(); j++)
        {
            ...
            for (int ik = 0; ik < (int)kpts.size(); ik++)
            {
                ...
                {
                    dist = pow(kpts_par_[i][j].pt.x - kpts[ik].pt.x, 2)
                           + pow(kpts_par_[i][j].pt.y - kpts[ik].pt.y, 2);
            ...
            if (is_extremum == true)
            {
                ...
                if (left_x < 0 || right_x >= evolution_[level].Ldet.cols || up_y < 0
                    || down_y >= evolution_[level].Ldet.rows)
                {
                    is_out = true;
                }
...
{
public:
    KAZE_Descriptor_Invoker(std::vector<KeyPoint>& kpts, Mat& desc, std::vector<TEvolution>& evolution,
                            const KAZEOptions& options)
        : kpts_(&kpts), desc_(&desc), evolution_(&evolution), options_(options)
    {
    }
 ...
 */
void KAZEFeatures::Compute_Main_Orientation(KeyPoint& kpt, const std::vector<TEvolution>& evolution_,
                                            const KAZEOptions& options)
{
    ...
    {
        ang2 = (ang1 + (float)(CV_PI / 3.0) > (float)(2.0 * CV_PI) ? ang1 - (float)(5.0 * CV_PI / 3.0)
        ...
        for (size_t k = 0; k < Ang.size(); ++k)
        {
            ...
            else if (ang2 < ang1 && ((ang > 0 && ang < ang2) || (ang > ang1 && ang < (float)(2.0 * CV_PI))))
            {
                sumX += resX[k];
                sumY += resY[k];
            }
...
void KAZE_Descriptor_Invoker::Get_KAZE_Upright_Descriptor_128(const KeyPoint& kpt, float* desc) const
{
    ...
    while (i < pattern_size)
    {
        ...
        while (j < pattern_size)
        {
            ...
            len += (dxp * dxp + dxn * dxn + mdxp * mdxp + mdxn * mdxn + dyp * dyp + dyn * dyn + mdyp * mdyp
                    + mdyn * mdyn)
                   * gauss_s2 * gauss_s2;
