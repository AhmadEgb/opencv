...
void AKAZEFeatures::Allocate_Memory_Evolution(void)
{
    ...
    for (int i = 0, power = 1; i <= options_.omax - 1; i++, power *= 2)
    {
        ...
        for (int j = 0; j < options_.nsublevels; j++)
        {
            ...
            step.sigma_size = cvRound(step.esigma * options_.derivative_factor
...
*/
static inline void nld_step_scalar_one_lane(const Mat& Lt, const Mat& Lf, Mat& Lstep, float step_size,
                                            int row_begin, int row_end)
{
   ...
   */
    ...
    if (row == 0)
    {
        ...
        for (int j = 0; j < cols; j++)
        {
            step_r = (lf_c[j] + lf_c[j + 1]) * (lt_c[j + 1] - lt_c[j])
                     + (lf_c[j] + lf_c[j - 1]) * (lt_c[j - 1] - lt_c[j]) + (lf_c[j] + lf_b[j]) * (lt_b[j] - lt_c[j]);
    ...
    for (; row < middle_end; ++row)
    {
        ...
        step_r = (lf_c[0] + lf_c[1]) * (lt_c[1] - lt_c[0]) + (lf_c[0] + lf_b[0]) * (lt_b[0] - lt_c[0])
        ...
        for (int j = 0; j < cols; j++)
        {
            step_r = (lf_c[j] + lf_c[j + 1]) * (lt_c[j + 1] - lt_c[j])
                     ...
                     + (lf_c[j] + lf_b[j]) * (lt_b[j] - lt_c[j]) + (lf_c[j] + lf_a[j]) * (lt_a[j] - lt_c[j]);
            dst[j] = step_r * step_size;
        }
    ...
    if (row_end == Lt.rows)
    {
        ...
        {
            step_r = (lf_c[j] + lf_c[j + 1]) * (lt_c[j + 1] - lt_c[j])
                     + (lf_c[j] + lf_c[j - 1]) * (lt_c[j - 1] - lt_c[j]) + (lf_c[j] + lf_a[j]) * (lt_a[j] - lt_c[j]);
            dst[j] = step_r * step_size;
        }
...
#ifdef HAVE_OPENCL
static inline bool ocl_non_linear_diffusion_step(InputArray Lt_, InputArray Lf_, OutputArray Lstep_, float step_size)
{
    if (!Lt_.isContinuous())
        return false;
    ...
    return ker
        .args(ocl::KernelArg::ReadOnly(Lt), ocl::KernelArg::PtrReadOnly(Lf), ocl::KernelArg::PtrWriteOnly(Lstep),
              step_size)
        .run(2, globalSize, 0, true);
}
#endif // HAVE_OPENCL

static inline void non_linear_diffusion_step(InputArray Lt_, InputArray Lf_, OutputArray Lstep_, float step_size)
{
    CV_INSTRUMENT_REGION();
 ...
 */
static inline float compute_kcontrast(InputArray Lx_, InputArray Ly_, float perc, int nbins)
{
    CV_INSTRUMENT_REGION();
...
#ifdef HAVE_OPENCL
static inline bool ocl_pm_g2(InputArray Lx_, InputArray Ly_, OutputArray Lflow_, float kcontrast)
{
    UMat Lx = Lx_.getUMat();
    UMat Ly = Ly_.getUMat();
    ...
    return ker
        .args(ocl::KernelArg::PtrReadOnly(Lx), ocl::KernelArg::PtrReadOnly(Ly),
              ocl::KernelArg::PtrWriteOnly(Lflow), kcontrast, total)
        .run(1, globalSize, 0, true);
}
#endif // HAVE_OPENCL

static inline void compute_diffusivity(InputArray Lx, InputArray Ly, OutputArray Lflow, float kcontrast,
...
static inline void create_nonlinear_scale_space(InputArray image, const AKAZEOptions& options,
                                                const std::vector<std::vector<float>>& tsteps_evolution,
                                                std::vector<Evolution<MatType>>& evolution)
{
    CV_INSTRUMENT_REGION();
...
static inline void convertScalePyramid(const std::vector<Evolution<MatTypeSrc>>& src,
                                       std::vector<Evolution<MatTypeDst>>& dst)
{
    dst.resize(src.size());
...
#ifdef HAVE_OPENCL
static inline bool ocl_compute_determinant(InputArray Lxx_, InputArray Lxy_, InputArray Lyy_, OutputArray Ldet_,
                                           float sigma)
{
    ...
    return ker
        .args(ocl::KernelArg::PtrReadOnly(Lxx), ocl::KernelArg::PtrReadOnly(Lxy), ocl::KernelArg::PtrReadOnly(Lyy),
              ocl::KernelArg::PtrWriteOnly(Ldet), sigma, total)
        .run(1, globalSize, 0, true);
}
#endif // HAVE_OPENCL
 ...
 */
static inline void compute_determinant(InputArray Lxx_, InputArray Lxy_, InputArray Lyy_, OutputArray Ldet_,
                                       float sigma)
{
    CV_INSTRUMENT_REGION();
...
{
public:
    explicit DeterminantHessianResponse(std::vector<Evolution<MatType>>& ev) : evolution_(&ev) {}
 ...
 */
static inline void Compute_Determinant_Hessian_Response(UMatPyramid& evolution)
{
    CV_INSTRUMENT_REGION();
 ...
 * @note We use the Hessian determinant as the feature detector response
 */
static inline void Compute_Determinant_Hessian_Response(Pyramid& evolution)
{
    CV_INSTRUMENT_REGION();
 ...
 */
static inline bool find_neighbor_point(const int x, const int y, const Mat& mask, const int search_radius, int& idx)
{
...
{
public:
    explicit FindKeypointsSameScale(const Pyramid& ev, std::vector<Mat>& kpts, float dthreshold)
 ...
 */
void AKAZEFeatures::Do_Subpixel_Refinement(std::vector<Mat>& keypoints_by_layers,
                                           std::vector<KeyPoint>& output_keypoints)
{
    CV_INSTRUMENT_REGION();
...
{
public:
    Upright_MLDB_Full_Descriptor_Invoker(std::vector<KeyPoint>& kpts, Mat& desc, Pyramid& evolution,
                                         AKAZEOptions& options)
        : keypoints_(&kpts), descriptors_(&desc), evolution_(&evolution), options_(&options)
    {
    }
...
{
public:
    Upright_MLDB_Descriptor_Subset_Invoker(std::vector<KeyPoint>& kpts, Mat& desc, Pyramid& evolution,
                                           AKAZEOptions& options, Mat descriptorSamples, Mat descriptorBits)
        : keypoints_(&kpts), descriptors_(&desc), evolution_(&evolution), options_(&options),
          descriptorSamples_(descriptorSamples), descriptorBits_(descriptorBits)
    {
    }
...
public:
    ...
    void MLDB_Fill_Values(float* values, int sample_step, int level, float xf, float yf, float co, float si,
                          float scale) const;
...
{
public:
    MLDB_Descriptor_Subset_Invoker(std::vector<KeyPoint>& kpts, Mat& desc, Pyramid& evolution,
                                   AKAZEOptions& options, Mat descriptorSamples, Mat descriptorBits)
        : keypoints_(&kpts), descriptors_(&desc), evolution_(&evolution), options_(&options),
          descriptorSamples_(descriptorSamples), descriptorBits_(descriptorBits)
    {
...
void AKAZEFeatures::Compute_Descriptors(std::vector<KeyPoint>& kpts, OutputArray descriptors)
{
    ...
    case AKAZE::DESCRIPTOR_MLDB_UPRIGHT: // Upright descriptors, not invariant to rotation
    {
        if (options_.descriptor_size == 0)
            parallel_for_(Range(0, (int)kpts.size()),
                          Upright_MLDB_Full_Descriptor_Invoker(kpts, desc, evolution_, options_));
        else
            parallel_for_(Range(0, (int)kpts.size()),
                          Upright_MLDB_Descriptor_Subset_Invoker(kpts, desc, evolution_, options_,
                                                                 descriptorSamples_, descriptorBits_));
    }
    break;
    case AKAZE::DESCRIPTOR_MLDB:
    {
        if (options_.descriptor_size == 0)
            parallel_for_(Range(0, (int)kpts.size()),
                          MLDB_Full_Descriptor_Invoker(kpts, desc, evolution_, options_));
        else
            parallel_for_(Range(0, (int)kpts.size()),
                          MLDB_Descriptor_Subset_Invoker(kpts, desc, evolution_, options_, descriptorSamples_,
                                                         descriptorBits_));
    }
    break;
    }
 ...
 */
static inline void Sample_Derivative_Response_Radius6(const Mat& Lx, const Mat& Ly, const int x0, const int y0,
                                                      const int scale, float* resX, float* resY)
{
    /* ************************************************************************* */
 ...
 */
static inline void quantized_counting_sort(const float a[], const int n, const float quantum, const int nkeys,
 ...
 */
static inline void Compute_Main_Orientation(KeyPoint& kpt, const Pyramid& evolution)
{
...
{
public:
    ComputeKeypointOrientation(std::vector<KeyPoint>& kpts, const Pyramid& evolution)
        : keypoints_(&kpts), evolution_(&evolution)
    {
    }
 ...
 * ECCV 2008
 */
void MSURF_Upright_Descriptor_64_Invoker::Get_MSURF_Upright_Descriptor_64(const KeyPoint& kpt, float* desc,
                                                                          int desc_size) const
{
        ...
        while (j < pattern_size)
        {
            ...
            for (int k = i; k < i + 9; k++)
            {
                for (int l = j; l < j + 9; l++)
                {
                    ...
                    rx = (1.0f - fx) * (1.0f - fy) * res1 + fx * (1.0f - fy) * res2 + (1.0f - fx) * fy * res3
                         + fx * fy * res4;
                    ...
                    ry = (1.0f - fx) * (1.0f - fy) * res1 + fx * (1.0f - fy) * res2 + (1.0f - fx) * fy * res3
                         + fx * fy * res4;
 ...
 * @param desc Descriptor vector
 */
void Upright_MLDB_Full_Descriptor_Invoker::Get_Upright_MLDB_Full_Descriptor(const KeyPoint& kpt,
                                                                            unsigned char* desc, int desc_size) const
{
    ...
    const int sample_step[3] = {pattern_size, divUp(pattern_size * 2, 3), divUp(pattern_size, 2)};
...
void MLDB_Full_Descriptor_Invoker::MLDB_Fill_Values(float* values, int sample_step, const int level, float xf,
...
void MLDB_Full_Descriptor_Invoker::MLDB_Binary_Comparisons(float* values, unsigned char* desc, int count,
                                                           int& dpos) const
{
    int chan = options_->descriptor_channels;
 ...
 * @param desc Descriptor vector
 */
void MLDB_Full_Descriptor_Invoker::Get_MLDB_Full_Descriptor(const KeyPoint& kpt, unsigned char* desc,
                                                            int desc_size) const
{
    ...
    const int sample_step[3] = {// static_cast<int>(ceil(pattern_size * size_mult[lvl]))
                                pattern_size, divUp(pattern_size * 2, 3), divUp(pattern_size, 2)};
 ...
 * @param desc Descriptor vector
 */
void MLDB_Descriptor_Subset_Invoker::Get_MLDB_Descriptor_Subset(const KeyPoint& kpt, unsigned char* desc,
                                                                int desc_size) const
{
    ...
    CV_Assert((pattern_size & 1) == 0);
    const int sample_steps[3] = {pattern_size, divUp(pattern_size * 2, 3), divUp(pattern_size, 2)};
 ...
 * @param desc Descriptor vector
 */
void Upright_MLDB_Descriptor_Subset_Invoker::Get_Upright_MLDB_Descriptor_Subset(const KeyPoint& kpt,
                                                                                unsigned char* desc,
                                                                                int desc_size) const
{
 ...
 */
void generateDescriptorSubsample(Mat& sampleList, Mat& comparisons, int nbits, int pattern_size, int nchannels)
{
