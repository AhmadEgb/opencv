...
#        ifndef vec_rsqrt
VSX_FINLINE(vec_float4) vec_rsqrt(const vec_float4& a) { return vec_div(vec_float4_sp(1), vec_sqrt(a)); }

VSX_FINLINE(vec_double2) vec_rsqrt(const vec_double2& a) { return vec_div(vec_double2_sp(1), vec_sqrt(a)); }
#        endif
...
VSX_FINLINE(vec_dword2) vec_ctslo(const vec_float4& a) { return vec_ctsl(vec_cvfo(a)); }
...
VSX_FINLINE(vec_udword2) vec_ctulo(const vec_float4& a) { return vec_ctul(vec_cvfo(a)); }
...
#    if defined(__clang__) || defined(__IBMCPP__)
VSX_FINLINE(vec_udword2) vec_splats(uint64 v) { return vec_splats((unsigned long long)v); }

VSX_FINLINE(vec_dword2) vec_splats(int64 v) { return vec_splats((long long)v); }

VSX_FINLINE(vec_udword2) vec_promote(uint64 a, int b) { return vec_promote((unsigned long long)a, b); }

VSX_FINLINE(vec_dword2) vec_promote(int64 a, int b) { return vec_promote((long long)a, b); }
#    endif
...
VSX_FINLINE(vec_dword2) vsx_ld2(long o, const int64* p) { return vsx_ldf(VSX_OFFSET(o, p), (long long*)p); }
...
#    define VSX_IMPL_LOAD_L8(Tvec, Tp) \
        VSX_FINLINE(Tvec) vec_ld_l8(const Tp* p) { return ((Tvec)vec_promote(*((uint64*)p), 0)); }
...
#    define VSX_IMPL_UNPACKU(rt, rg, zero) \
        VSX_FINLINE(rt) vec_unpacklu(const rg& a) { return (rt)(vec_mergel(a, zero)); } \
        VSX_FINLINE(rt) vec_unpackhu(const rg& a) { return (rt)(vec_mergeh(a, zero)); }
