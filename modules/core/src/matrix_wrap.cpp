...
cuda::GpuMat _InputArray::getGpuMat() const
{
    ...
    {
        CV_Error(cv::Error::StsNotImplemented,
                 "You should explicitly call mapDevice/unmapDevice methods for ogl::Buffer object");
    }
...
_InputArray::KindFlag _InputArray::kind() const { return flags & KIND_MASK; }

int _InputArray::rows(int i) const { return size(i).height; }

int _InputArray::cols(int i) const { return size(i).width; }
...
int _InputArray::depth(int i) const { return CV_MAT_DEPTH(type(i)); }

int _InputArray::channels(int i) const { return CV_MAT_CN(type(i)); }
...
bool _InputArray::isContinuous(int i) const
{
    ...
    if (k == EXPR || k == MATX || k == STD_VECTOR || k == STD_ARRAY || k == NONE || k == STD_VECTOR_VECTOR
        || k == STD_BOOL_VECTOR)
        return true;
...
void _OutputArray::create(Size _sz, int mtype, int i, bool allowTransposed,
...
void _OutputArray::create(int _rows, int _cols, int mtype, int i, bool allowTransposed,
                          _OutputArray::DepthMask fixedDepthMask) const
{
    _InputArray::KindFlag k = kind();
    if (k == MAT && i < 0 && !allowTransposed && fixedDepthMask == 0)
...
void _OutputArray::create(int d, const int* sizes, int mtype, int i, bool allowTransposed,
                          _OutputArray::DepthMask fixedDepthMask) const
{
    int sizebuf[2];
    if (d == 1)
    ...
    if (k == UMAT)
    {
        ...
        if (allowTransposed)
        {
            ...
            if (d == 2 && m.dims == 2 && !m.empty() && m.type() == mtype && m.rows == sizes[1] && m.cols == sizes[0])
                return;
        }
    ...
    if (k == MATX)
    {
        ...
        CV_Assert(d == 2
                  && ((sizes[0] == sz.height && sizes[1] == sz.width)
    ...
    if (k == STD_VECTOR_UMAT)
    {
        ...
        if (allowTransposed)
        {
            ...
            if (d == 2 && m.dims == 2 && m.u && m.type() == mtype && m.rows == sizes[1] && m.cols == sizes[0])
                return;
        }
