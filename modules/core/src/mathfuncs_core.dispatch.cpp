...
void fastAtan32f(const float* Y, const float* X, float* angle, int len, bool angleInDegrees)
{
    ...
    CV_CPU_DISPATCH(fastAtan32f, (Y, X, angle, len, angleInDegrees), CV_CPU_DISPATCH_MODES_ALL);
}
...
void fastAtan64f(const double* Y, const double* X, double* angle, int len, bool angleInDegrees)
{
    ...
    CV_CPU_DISPATCH(fastAtan64f, (Y, X, angle, len, angleInDegrees), CV_CPU_DISPATCH_MODES_ALL);
}
...
void magnitude32f(const float* x, const float* y, float* mag, int len)
{
    ...
    CV_CPU_DISPATCH(magnitude32f, (x, y, mag, len), CV_CPU_DISPATCH_MODES_ALL);
}
...
void magnitude64f(const double* x, const double* y, double* mag, int len)
{
    ...
    // SSE42 performance issues
    CV_IPP_RUN(IPP_VERSION_X100 > 201800 || cv::ipp::getIppTopFeatures() != ippCPUID_SSE42,
               CV_INSTRUMENT_FUN_IPP(ippsMagnitude_64f, x, y, mag, len) >= 0);

    CV_CPU_DISPATCH(magnitude64f, (x, y, mag, len), CV_CPU_DISPATCH_MODES_ALL);
}
...
void invSqrt32f(const float* src, float* dst, int len)
{
    ...
    CV_CPU_DISPATCH(invSqrt32f, (src, dst, len), CV_CPU_DISPATCH_MODES_ALL);
}
...
void invSqrt64f(const double* src, double* dst, int len)
{
    ...
    CV_CPU_DISPATCH(invSqrt64f, (src, dst, len), CV_CPU_DISPATCH_MODES_ALL);
}
...
void sqrt32f(const float* src, float* dst, int len)
{
    ...
    CV_CPU_DISPATCH(sqrt32f, (src, dst, len), CV_CPU_DISPATCH_MODES_ALL);
}
...
void sqrt64f(const double* src, double* dst, int len)
{
    ...
    CV_CPU_DISPATCH(sqrt64f, (src, dst, len), CV_CPU_DISPATCH_MODES_ALL);
}
...
void exp32f(const float* src, float* dst, int n)
{
    ...
    CV_CPU_DISPATCH(exp32f, (src, dst, n), CV_CPU_DISPATCH_MODES_ALL);
}
...
void exp64f(const double* src, double* dst, int n)
{
    ...
    CV_CPU_DISPATCH(exp64f, (src, dst, n), CV_CPU_DISPATCH_MODES_ALL);
}
...
void log32f(const float* src, float* dst, int n)
{
    ...
    CV_CPU_DISPATCH(log32f, (src, dst, n), CV_CPU_DISPATCH_MODES_ALL);
}
...
void log64f(const double* src, double* dst, int n)
{
    ...
    CV_CPU_DISPATCH(log64f, (src, dst, n), CV_CPU_DISPATCH_MODES_ALL);
}

//=============================================================================
// for compatibility with 3.0

void exp(const float* src, float* dst, int n) { exp32f(src, dst, n); }

void exp(const double* src, double* dst, int n) { exp64f(src, dst, n); }

void log(const float* src, float* dst, int n) { log32f(src, dst, n); }

void log(const double* src, double* dst, int n) { log64f(src, dst, n); }

void magnitude(const float* x, const float* y, float* dst, int n) { magnitude32f(x, y, dst, n); }

void magnitude(const double* x, const double* y, double* dst, int n) { magnitude64f(x, y, dst, n); }

void sqrt(const float* src, float* dst, int len) { sqrt32f(src, dst, len); }

void sqrt(const double* src, double* dst, int len) { sqrt64f(src, dst, len); }

void invSqrt(const float* src, float* dst, int len) { invSqrt32f(src, dst, len); }

void invSqrt(const double* src, double* dst, int len) { invSqrt64f(src, dst, len); }
