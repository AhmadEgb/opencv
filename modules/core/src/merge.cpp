...
static void vecmerge_(const T** src, T* dst, int len, int cn)
{
...
template<typename T>
static void merge_(const T** src, T* dst, int len, int cn)
{
    int k = cn % 4 ? cn % 4 : 4;
    int i, j;
    if (k == 1)
...
#    ifdef HAVE_IPP_IW
    ...
    if (mv[0].dims <= 2)
    {
        ...
        return CV_INSTRUMENT_FUN_IPP(llwiCopyMerge, srcPtrs, (int)srcStep, dst.ptr(), (int)dst.step, size,
                                     (int)mv[0].elemSize1(), channels, 0)
               >= 0;
    }
    else
    {
        ...
        {
            if (CV_INSTRUMENT_FUN_IPP(llwiCopyMerge, (const void**)&ptrs[1], 0, ptrs[0], 0, size,
                                      (int)mv[0].elemSize1(), channels, 0)
                < 0)
                return false;
        }
...
void cv::merge(const Mat* mv, size_t n, OutputArray _dst)
{
    ...
    size_t blocksize = std::min((size_t)CV_SPLIT_MERGE_MAX_BLOCK_SIZE(cn),
                                cn <= 4 ? total : std::min(total, blocksize0));
...
static bool ocl_merge(InputArrayOfArrays _mv, OutputArray _dst)
{
    ...
    int type = src[0].type(), depth = CV_MAT_DEPTH(type), rowsPerWI = ocl::Device::getDefault().isIntel() ? 4 : 1;
    ...
    ocl::Kernel k("merge", ocl::core::split_merge_oclsrc,
                  format("-D OP_MERGE -D cn=%d -D T=%s -D DECLARE_SRC_PARAMS_N=%s"
                         ...
                         dcn, ocl::memopTypeToStr(depth), srcargs.c_str(), indexdecl.c_str(), processelem.c_str(),
                         cndecl.c_str()));
    if (k.empty())
        return false;
...
void cv::merge(InputArrayOfArrays _mv, OutputArray _dst)
{
    ...
    CV_OCL_RUN(_mv.isUMatVector() && _dst.isUMat(), ocl_merge(_mv, _dst))
