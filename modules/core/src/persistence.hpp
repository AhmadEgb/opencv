...
public:
    ...
    void write(_to_binary_convertor_t& convertor, const char* dt);
...
#define CV_CHECK_FILE_STORAGE(fs) \
    { \
        if (!CV_IS_FILE_STORAGE(fs)) \
            CV_Error((fs) ? CV_StsBadArg : CV_StsNullPtr, "Invalid pointer to file storage"); \
    }
...
#define CV_PARSE_ERROR(errmsg) icvParseError(fs, CV_Func, (errmsg), __FILE__, __LINE__)
...
typedef void (*CvStartWriteStruct)(struct CvFileStorage* fs, const char* key, int struct_flags,
...
inline bool cv_isalpha(char c) { return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z'); }

inline bool cv_isdigit(char c) { return '0' <= c && c <= '9'; }

inline bool cv_isspace(char c) { return (9 <= c && c <= 13) || c == ' '; }
...
void CV_NORETURN icvParseError(CvFileStorage* fs, const char* func_name, const char* err_msg,
...
void icvXMLStartWriteStruct(CvFileStorage* fs, const char* key, int struct_flags,
...
void icvYMLStartWriteStruct(CvFileStorage* fs, const char* key, int struct_flags,
                            const char* type_name CV_DEFAULT(0));
...
void icvJSONStartWriteStruct(CvFileStorage* fs, const char* key, int struct_flags,
                             const char* type_name CV_DEFAULT(0));
