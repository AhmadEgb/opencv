...
template<typename T>
static void mixChannels_(const T** src, const int* sdelta, T** dst, const int* ddelta, int len, int npairs)
{
    int i, k;
    for (k = 0; k < npairs; k++)
    {
...
static void mixChannels8u(const uchar** src, const int* sdelta, uchar** dst, const int* ddelta, int len, int npairs)
{
    mixChannels_(src, sdelta, dst, ddelta, len, npairs);
}

static void mixChannels16u(const ushort** src, const int* sdelta, ushort** dst, const int* ddelta, int len,
                           int npairs)
{
    mixChannels_(src, sdelta, dst, ddelta, len, npairs);
}

static void mixChannels32s(const int** src, const int* sdelta, int** dst, const int* ddelta, int len, int npairs)
{
    mixChannels_(src, sdelta, dst, ddelta, len, npairs);
}

static void mixChannels64s(const int64** src, const int* sdelta, int64** dst, const int* ddelta, int len, int npairs)
{
    mixChannels_(src, sdelta, dst, ddelta, len, npairs);
}

typedef void (*MixChannelsFunc)(const uchar** src, const int* sdelta, uchar** dst, const int* ddelta, int len,
                                int npairs);

static MixChannelsFunc getMixchFunc(int depth)
{
    static MixChannelsFunc mixchTab[] = {(MixChannelsFunc)mixChannels8u,  (MixChannelsFunc)mixChannels8u,
...
void cv::mixChannels(const Mat* src, size_t nsrcs, Mat* dst, size_t ndsts, const int* fromTo, size_t npairs)
{
    ...
    AutoBuffer<uchar> buf((nsrcs + ndsts + 1) * (sizeof(Mat*) + sizeof(uchar*))
                          + npairs * (sizeof(uchar*) * 2 + sizeof(int) * 6));
    ...
    for (i = 0; i < npairs; i++)
    {
        ...
        if (i0 >= 0)
        {
            ...
            tab[i * 4] = (int)j;
            tab[i * 4 + 1] = (int)(i0 * esz1);
            sdelta[i] = src[j].channels();
        }
        else
        {
            tab[i * 4] = (int)(nsrcs + ndsts);
            tab[i * 4 + 1] = 0;
            sdelta[i] = 0;
        }
        ...
        tab[i * 4 + 2] = (int)(j + nsrcs);
        tab[i * 4 + 3] = (int)(i1 * esz1);
...
static bool ocl_mixChannels(InputArrayOfArrays _src, InputOutputArrayOfArrays _dst, const int* fromTo, size_t npairs)
{
    ...
    int depth = src[0].depth(), esz = CV_ELEM_SIZE(depth), rowsPerWI = ocl::Device::getDefault().isIntel() ? 4 : 1;
    ...
    ocl::Kernel k("mixChannels", ocl::core::mixchannels_oclsrc,
                  format("-D T=%s -D DECLARE_INPUT_MAT_N=%s -D DECLARE_OUTPUT_MAT_N=%s"
                         ...
                         ocl::memopTypeToStr(depth), declsrc.c_str(), decldst.c_str(), declproc.c_str(),
                         indexdecl.c_str(), declcn.c_str()));
    if (k.empty())
        return false;
...
void cv::mixChannels(InputArrayOfArrays src, InputOutputArrayOfArrays dst, const int* fromTo, size_t npairs)
{
    ...
    CV_OCL_RUN(dst.isUMatVector(), ocl_mixChannels(src, dst, fromTo, npairs))

    bool src_is_mat = src.kind() != _InputArray::STD_VECTOR_MAT && src.kind() != _InputArray::STD_ARRAY_MAT
                      && src.kind() != _InputArray::STD_VECTOR_VECTOR && src.kind() != _InputArray::STD_VECTOR_UMAT;
    bool dst_is_mat = dst.kind() != _InputArray::STD_VECTOR_MAT && dst.kind() != _InputArray::STD_ARRAY_MAT
                      && dst.kind() != _InputArray::STD_VECTOR_VECTOR && dst.kind() != _InputArray::STD_VECTOR_UMAT;
    int i;
...
void cv::mixChannels(InputArrayOfArrays src, InputOutputArrayOfArrays dst, const std::vector<int>& fromTo)
{
    ...
    CV_OCL_RUN(dst.isUMatVector(), ocl_mixChannels(src, dst, &fromTo[0], fromTo.size() >> 1))
...
#    ifdef HAVE_IPP_IW
    ...
    if (src.dims <= 2)
    {
        ...
        return CV_INSTRUMENT_FUN_IPP(llwiCopyChannel, src.ptr(), (int)src.step, srcChannels, channel, dst.ptr(),
                                     (int)dst.step, dstChannels, 0, size, (int)src.elemSize1())
               >= 0;
    }
    else
    {
        ...
        {
            if (CV_INSTRUMENT_FUN_IPP(llwiCopyChannel, ptrs[0], 0, srcChannels, channel, ptrs[1], 0, dstChannels,
                                      0, size, (int)src.elemSize1())
                < 0)
                return false;
        }
        return true;
...
#    ifdef HAVE_IPP_IW
    ...
    if (src.dims <= 2)
    {
        ...
        return CV_INSTRUMENT_FUN_IPP(llwiCopyChannel, src.ptr(), (int)src.step, srcChannels, 0, dst.ptr(),
                                     (int)dst.step, dstChannels, channel, size, (int)src.elemSize1())
               >= 0;
    }
    else
    {
        ...
        for (size_t i = 0; i < it.nplanes; i++, ++it)
        {
            if (CV_INSTRUMENT_FUN_IPP(llwiCopyChannel, ptrs[0], 0, srcChannels, 0, ptrs[1], 0, dstChannels,
                                      channel, size, (int)src.elemSize1())
                < 0)
                return false;
        }
        return true;
