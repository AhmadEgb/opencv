               ...
               * (c == 'u' || c == 'c'
                      ...
                      : c == 'w' || c == 's'
                            ...
                            : c == 'i'
                                  ? sizeof(int)
                                  : c == 'f' ? sizeof(float)
                                             : c == 'd' ? sizeof(double) : c == 'r' ? sizeof(void*) : (size_t)0);
}

FileStorage::FileStorage() { state = UNDEFINED; }
...
FileStorage::FileStorage(CvFileStorage* _fs, bool owning)
{
    if (owning)
        ...
        fs = Ptr<CvFileStorage>(Ptr<CvFileStorage>(), _fs);
...
bool FileStorage::isOpened() const { return fs && fs->is_opened; }
...
FileStorage& operator<<(FileStorage& fs, const String& str)
{
    ...
    enum
    {
        NAME_EXPECTED = FileStorage::NAME_EXPECTED,
    ...
    if (*_str == '}' || *_str == ']')
    {
        ...
        if ((*_str == ']' ? '[' : '{') != fs.structs.back())
            CV_Error_(CV_StsError, ("The closing '%c' does not match the opening '%c'", *_str, fs.structs.back()));
        fs.structs.pop_back();
        fs.state = fs.structs.empty() || fs.structs.back() == '{' ? INSIDE_MAP + NAME_EXPECTED : VALUE_EXPECTED;
        cvEndWriteStruct(*fs);
        fs.elname = String();
    }
    ...
    else if ((fs.state & 3) == VALUE_EXPECTED)
    {
        if (*_str == '{' || *_str == '[')
        {
            ...
            fs.state = flags == CV_NODE_MAP ? INSIDE_MAP + NAME_EXPECTED : VALUE_EXPECTED;
            if (*_str == ':')
            {
                flags |= CV_NODE_FLOW;
                _str++;
            }
            cvStartWriteStruct(*fs, fs.elname.size() > 0 ? fs.elname.c_str() : 0, flags, *_str ? _str : 0);
            fs.elname = String();
        }
        else
        {
            write(fs, fs.elname,
                  (_str[0] == '\\' && (_str[1] == '{' || _str[1] == '}' || _str[1] == '[' || _str[1] == ']'))
                      ? String(_str + 1)
                      : str);
...
void FileStorage::write(const String& name, int val) { *this << name << val; }

void FileStorage::write(const String& name, double val) { *this << name << val; }

void FileStorage::write(const String& name, const String& val) { *this << name << val; }

void FileStorage::write(const String& name, const Mat& val) { *this << name << val; }
...
{
    return isSeq() ? FileNode(fs, (CvFileNode*)cvGetSeqElem(node->data.seq, i)) : i == 0 ? *this : FileNode();
}
...
FileNodeIterator& FileNodeIterator::operator-=(int ofs) { return operator+=(-ofs); }
...
void writeScalar(FileStorage& fs, int value) { cvWriteInt(*fs, 0, value); }

void writeScalar(FileStorage& fs, float value) { cvWriteReal(*fs, 0, value); }

void writeScalar(FileStorage& fs, double value) { cvWriteReal(*fs, 0, value); }

void writeScalar(FileStorage& fs, const String& value) { cvWriteString(*fs, 0, value.c_str()); }
...
internal::WriteStructContext::WriteStructContext(FileStorage& _fs, const String& name, int flags,
                                                 const String& typeName)
    : fs(&_fs)
{
    cvStartWriteStruct(**fs, !name.empty() ? name.c_str() : 0, flags, !typeName.empty() ? typeName.c_str() : 0);
    fs->elname = String();
...
{
    value = !node.node ? default_value
                       : CV_NODE_IS_STRING(node.node->tag) ? std::string(node.node->data.str.ptr) : default_value;
}
