...
static void vecsplit_(const T* src, T** dst, int len, int cn)
{
...
#    ifdef HAVE_IPP_IW
    ...
    if (src.dims <= 2)
    {
        ...
        return CV_INSTRUMENT_FUN_IPP(llwiCopySplit, src.ptr(), (int)src.step, dstPtrs, (int)dstStep, size,
                                     (int)src.elemSize1(), channels, 0)
               >= 0;
    }
    else
    {
        ...
        {
            if (CV_INSTRUMENT_FUN_IPP(llwiCopySplit, ptrs[0], 0, (void**)&ptrs[1], 0, size, (int)src.elemSize1(),
                                      channels, 0)
                < 0)
                return false;
        }
...
static bool ocl_split(InputArray _m, OutputArrayOfArrays _mv)
{
    ...
    ocl::Kernel k("split", ocl::core::split_merge_oclsrc,
                  format("-D T=%s -D OP_SPLIT -D cn=%d -D DECLARE_DST_PARAMS=%s"
                         ...
                         ocl::memopTypeToStr(depth), cn, dstargs.c_str(), processelem.c_str(), indexdecl.c_str()));
    if (k.empty())
        return false;
...
void cv::split(InputArray _m, OutputArrayOfArrays _mv)
{
    ...
    CV_OCL_RUN(_m.dims() <= 2 && _mv.isUMatVector(), ocl_split(_m, _mv))
