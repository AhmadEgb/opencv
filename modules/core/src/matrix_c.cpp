...
CV_IMPL CvScalar cvTrace(const CvArr* arr) { return cvScalar(cv::trace(cv::cvarrToMat(arr))); }
...
CV_IMPL CvArr* cvRange(CvArr* arr, double start, double end)
{
    ...
    if (type == CV_32SC1)
    {
        ...
        if (fabs(val - ival) < DBL_EPSILON && fabs(delta - idelta) < DBL_EPSILON)
        {
...
CV_IMPL int cvKMeans2(const CvArr* _samples, int cluster_count, CvArr* _labels, CvTermCriteria termcrit,
                      int attempts, CvRNG*, int flags, CvArr* _centers, double* _compactness)
{
    ...
    CV_Assert(labels.isContinuous() && labels.type() == CV_32S && (labels.cols == 1 || labels.rows == 1)
              && labels.cols + labels.rows - 1 == data.rows);

    double compactness = cv::kmeans(data, cluster_count, labels, termcrit, attempts, flags,
                                    _centers ? cv::_OutputArray(centers) : cv::_OutputArray());
    if (_compactness)
