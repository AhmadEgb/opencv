...
static char* icvYMLParseValue(CvFileStorage* fs, char* ptr, CvFileNode* node, int parent_flags, int min_indent)
{
    ...
    else if (cv_isdigit(c) || ((c == '-' || c == '+') && (cv_isdigit(d) || d == '.'))
    ...
    else if (c == '[' || c == '{') // collection as a flow
    {
        ...
        icvFSCreateCollection(fs, CV_NODE_TYPE(struct_flags) + (node->info ? CV_NODE_USER : 0), node);
    ...
    else
    {
        ...
        force_string:
            ...
            do
                c = *++endptr;
            while (cv_isprint(c) && (!is_parent_flow || (c != ',' && c != '}' && c != ']'))
        ...
        icvFSCreateCollection(fs, struct_flags + (node->info ? CV_NODE_USER : 0), node);
...
void icvYMLParse(CvFileStorage* fs)
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (*ptr == '%')
            {
                if (memcmp(ptr, "%YAML", 5) == 0 && memcmp(ptr, "%YAML:1.", 8) != 0
    ...
    if (quote || len == 0 || str[0] != str[len - 1] || (str[0] != '\"' && str[0] != '\''))
    {
        ...
        *data++ = '\"';
        for (i = 0; i < len; i++)
        {
            ...
            if (!need_quote && !cv_isalnum(c) && c != '_' && c != ' ' && c != '-' && c != '(' && c != ')'
                && c != '/' && c != '+' && c != ';')
                need_quote = 1;
