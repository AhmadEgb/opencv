...
static void icvXMLGetMultilineStringContent(CvFileStorage* fs, char* ptr, char*& beg, char*& end)
{
...
static char* icvXMLParseBase64(CvFileStorage* fs, char* ptr, CvFileNode* node)
{
    ...
    if (base64_buffer.empty() || !base64::base64_valid(base64_buffer.data(), 0U, base64_buffer.size()))
...
static char* icvXMLParseTag(CvFileStorage* fs, char* ptr, CvStringHashNode** _tag, CvAttrList** _list,
                            int* _tag_type);

static char* icvXMLParseValue(CvFileStorage* fs, char* ptr, CvFileNode* node,
                              int value_type CV_DEFAULT(CV_NODE_NONE))
{
    ...
    for (;;)
    {
        ...
        if (cv_isspace(c) || c == '\0'
            || (c == '<' && ptr[1] == '!'
                && ptr[2] == '-')) // FIXIT ptr[1], ptr[2] - out of bounds read without check or data fetch (#11061)
        {
        ...
        else
        {
            ...
            if (value_type != CV_NODE_STRING
                && (cv_isdigit(c) || ((c == '-' || c == '+') && (cv_isdigit(d) || d == '.'))
                    ...
                    if (!cv_isalnum(c))
                    {
                        ...
                        else if (c == '&')
                        {
                            if (*++ptr == '#')
                            {
                                ...
                                if ((unsigned)val > (unsigned)255 || !endptr || *endptr != ';')
    ...
    if ((CV_NODE_TYPE(node->tag) == CV_NODE_NONE
         || (CV_NODE_TYPE(node->tag) != value_type && !CV_NODE_IS_COLLECTION(node->tag)))
        && CV_NODE_IS_COLLECTION(value_type))
    {
        icvFSCreateCollection(fs, CV_NODE_IS_MAP(value_type) ? CV_NODE_MAP : CV_NODE_SEQ, node);
    }

    if (value_type != CV_NODE_NONE && value_type != CV_NODE_TYPE(node->tag))
    ...
    for (;;)
    {
        ...
        else if (c == '?' && tag_type == CV_XML_HEADER_TAG)
        {
            ...
            break;
                 ...
                 && tag_type == CV_XML_OPENING_TAG) // FIXIT ptr[1] - out of bounds read without check
        {
...
void icvXMLParse(CvFileStorage* fs)
{
    ...
    while (*ptr != '\0')
    {
        ...
        if (*ptr != '\0')
        {
            ...
            ptr = icvXMLParseTag(fs, ptr, &key, &list, &tag_type);
            if (tag_type != CV_XML_OPENING_TAG || !key || strcmp(key->str.ptr, "opencv_storage") != 0)
...
static void icvXMLWriteTag(CvFileStorage* fs, const char* key, int tag_type, CvAttrList list)
{
    char* ptr = fs->buffer;
    int i, len = 0;
...
void icvXMLStartWriteStruct(CvFileStorage* fs, const char* key, int struct_flags, const char* type_name)
{
    ...
    if (!CV_NODE_IS_COLLECTION(struct_flags))
        CV_Error(CV_StsBadArg, "Some collection type: CV_NODE_SEQ or CV_NODE_MAP must be specified");
...
void icvXMLWriteScalar(CvFileStorage* fs, const char* key, const char* data, int len)
{
    ...
    if (CV_NODE_IS_MAP(fs->struct_flags) || (!CV_NODE_IS_COLLECTION(fs->struct_flags) && key))
    {
...
void icvXMLWriteString(CvFileStorage* fs, const char* key, const char* str, int quote)
{
    ...
    if (quote || len == 0 || str[0] != '\"' || str[0] != str[len - 1])
    {
        ...
        *data++ = '\"';
        ...
        if (!need_quote && (cv_isdigit(str[0]) || str[0] == '+' || str[0] == '-' || str[0] == '.'))
            need_quote = 1;
