    ...
    while (i < len0)
    {
        ...
        while (j < std::min(len0, i + 32766 * v_int16::nlanes))
        {
            ...
            for (; k < std::min(len0, j + 127 * v_int8::nlanes); k += v_int8::nlanes)
                v_sum8 += v_one
                          & v_pack(v_reinterpret_as_s16(vx_load(src + k) == v_zero),
                                   v_reinterpret_as_s16(vx_load(src + k + v_uint16::nlanes) == v_zero));
            v_int16 part1, part2;
            v_expand(v_sum8, part1, part2);
            v_sum16 += part1 + part2;
            j = k;
...
static int countNonZero32s(const int* src, int len)
{
    ...
    while (i < len0)
    {
        ...
        while (j < std::min(len0, i + 32766 * v_int16::nlanes))
        {
            ...
            for (; k < std::min(len0, j + 127 * v_int8::nlanes); k += v_int8::nlanes)
                v_sum8 += v_one
                          & v_pack(v_pack(vx_load(src + k) == v_zero, vx_load(src + k + v_int32::nlanes) == v_zero),
                                   v_pack(vx_load(src + k + 2 * v_int32::nlanes) == v_zero,
                                          vx_load(src + k + 3 * v_int32::nlanes) == v_zero));
            v_int16 part1, part2;
            v_expand(v_sum8, part1, part2);
            v_sum16 += part1 + part2;
            j = k;
...
static int countNonZero32f(const float* src, int len)
{
    ...
    while (i < len0)
    {
        ...
        while (j < std::min(len0, i + 32766 * v_int16::nlanes))
        {
            ...
            for (; k < std::min(len0, j + 127 * v_int8::nlanes); k += v_int8::nlanes)
                v_sum8 += v_one
                          & v_pack(v_pack(v_reinterpret_as_s32(vx_load(src + k) == v_zero),
                                          v_reinterpret_as_s32(vx_load(src + k + v_float32::nlanes) == v_zero)),
                                   v_pack(v_reinterpret_as_s32(vx_load(src + k + 2 * v_float32::nlanes) == v_zero),
                                          v_reinterpret_as_s32(vx_load(src + k + 3 * v_float32::nlanes) == v_zero)));
            v_int16 part1, part2;
            v_expand(v_sum8, part1, part2);
            v_sum16 += part1 + part2;
            j = k;
...
static int countNonZero64f(const double* src, int len) { return countNonZero_(src, len); }
...
static bool ocl_countNonZero(InputArray _src, int& res)
{
    ...
    ocl::Kernel k("reduce", ocl::core::reduce_oclsrc,
                  format("-D srcT=%s -D srcT1=%s -D cn=1 -D OP_COUNT_NON_ZERO"
                         ...
                         ocl::typeToStr(CV_MAKE_TYPE(depth, kercn)), ocl::typeToStr(depth), (int)wgs, kercn,
    ...
    k.args(ocl::KernelArg::ReadOnlyNoSize(src), src.cols, (int)src.total(), dbsize,
...
#    endif
    ...
    if (src.dims <= 2)
    {
        ...
        if (depth == CV_8U)
            status = CV_INSTRUMENT_FUN_IPP(ippiCountInRange_8u_C1R, (const Ipp8u*)src.ptr(), (int)src.step, size,
                                           &count, 0, 0);
        else if (depth == CV_32F)
            status = CV_INSTRUMENT_FUN_IPP(ippiCountInRange_32f_C1R, (const Ipp32f*)src.ptr(), (int)src.step, size,
                                           &count, 0, 0);
        else
            return false;
    ...
    else
    {
        ...
        {
            if (depth == CV_8U)
                status = CV_INSTRUMENT_FUN_IPP(ippiCountInRange_8u_C1R, it.planes->ptr<Ipp8u>(),
                                               (int)it.planes->step, size, &count, 0, 0);
            else if (depth == CV_32F)
                status = CV_INSTRUMENT_FUN_IPP(ippiCountInRange_32f_C1R, it.planes->ptr<Ipp32f>(),
                                               (int)it.planes->step, size, &count, 0, 0);
            else
                return false;
...
#ifdef HAVE_OPENCL
    CV_OCL_RUN_(OCL_PERFORMANCE_CHECK(_src.isUMat()) && _src.dims() <= 2, ocl_countNonZero(_src, res), res)
#endif
